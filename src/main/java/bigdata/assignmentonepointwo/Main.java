...
public class Main {
	public static void main(String[] args) throws IOException {
		...
		LogParser log0 = new LogParser("Site0-access.log");
		Thread thread0 = new Thread(log0);
		thread0.start();
		LogParser log1 = new LogParser("Site1-access.log");
		Thread thread1 = new Thread(log1);
		thread1.start();
		LogParser log2 = new LogParser("Site2-access.log");
		Thread thread2 = new Thread(log2);
		thread2.start();
		LogParser log3 = new LogParser("Site3-access.log");
		Thread thread3 = new Thread(log3);
		thread3.start();
		LogParser log4 = new LogParser("Site4-access.log");
		Thread thread4 = new Thread(log4);
		thread4.start();
		LogParser log5 = new LogParser("Site5-access.log");
